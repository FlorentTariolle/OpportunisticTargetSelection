...
Testing on settings where switch timing may matter more (larger $\epsilon$, CIFAR-10 with $K=10$) is a natural next step. An adaptive re-locking variant that releases the lock when the locked class drops in ranking could recover from bad early commits, and evaluating OTS under $L_2$ budgets would probe whether drift dynamics differ with smoother perturbations.
...
\label{tab:simba-s-sweep}\label{tab:square-s-sweep}
\small
